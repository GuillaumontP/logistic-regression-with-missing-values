...
for (NB in 1:nbsim)
{
  ...
  P.CC = c(data.obs2 %*% beta.small.cc + beta0.small.cc) > 0.5
  P.CC = as.integer(P.CC) 
  ###
  predCC = P.CC  
  ...
  {
    list.small.saem = miss.saem(X.obs, y, print_iter = FALSE, var_cal = TRUE, tau = tau)
    ...
    pred = round(pred_saem(X.test, list.small.saem$beta, list.small.saem$mu, list.small.saem$sig2))
    predSAEM = c(predSAEM,pred) 
    y.confusionSAEM = c(y.confusionSAEM, y.test)
    
    SAEM_tau[k,] = SAEM_tau[k,] + c(confusionMatrix(table(predSAEM,y.confusionSAEM))$table) #confusion matrix in vector
    k = k + 1 
  }
  k = 1
}
...
print(prop.table(confusionMatrix(table(predNAall, y.confusion))$table))
print("matrice de confusion sans se soucier des valeurs manquantes")
print(prop.table(confusionMatrix(table(predCCall, y.confusion))$table))
