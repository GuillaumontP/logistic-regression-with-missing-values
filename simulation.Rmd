...
for (NB in 1:nbsim)
{
  ...
  for (tau in seq(0.5,1, 0.1)) 
  {
    ...
    SAEM_tau[k,] = SAEM_tau[k,] + c(confusionMatrix(table(round(predSAEM),y.confusionSAEM))$table) #confusion matrix in vector
    k = k + 1 
  }
  k = 1
}
...
```
```{r ROC NA}
pre = prediction(predNAall, y.confusion)
...
```{r ROC SAEM}
pre = prediction(predSAEM, y.confusionSAEM)
prf <- performance(pre, measure = "tpr", x.measure = "fpr")
